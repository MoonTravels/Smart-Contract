...
/*
MoonTravels Token $MTLC is Hyper-deflationary token that rewards investors for holding tokens while also
...
contract MoonTravels is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private MAKAddress;
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma) {
        ...
        MAKAddress = payable(0x5B071c86dEA20FE0cc464017dc4b84fe73794b18);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(MAKAddress, transferredBalance.div(_liquidityFee).mul(MAKFee));
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(MAKFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= MAKFee, 'Marketing fee can not be less than 5 percent');
